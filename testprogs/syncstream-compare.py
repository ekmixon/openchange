...
def compare_dict_keys(left_dict, right_dict, label):
    ...
    if missing_keys := [x for x in left_dict if x not in right_dict]:
        print(
            f'< {label} keys found in the left dict but not in the right one: {", ".join(missing_keys)}'
        )


    if missing_keys := [x for x in right_dict if x not in left_dict]:
        print(
            f'> {label} keys found in the right dict but not in the left one: {", ".join(missing_keys)}'
        )
...
class SyncStream(object):
    ...
    def _parse(self):
        ...
        if line != "":
            ...
            if len_line > 0:
                if line[0] == "*":
                    if current_section is not None and current_prop_data:
